...
static int serverConnectOk()
{
    debugOut(0, "[\t%d] Server connect ok\r\n", osKernelSysTick());
}
static int serverDisconnect( int type )
{
    if( type == 1 )
    {
...
void protocolRun( FifoClass *dataIn, FifoClass *dataOut )
{
    static uint64_t packIndex = -1, packIndexBak = -1;
    uint16_t packCMD = 0;
    uint8_t errorCode;
    int PackLen;
    union {
...
int cliSOcketConnect( FifoClass *out )
{
    char buff[500];
    ...
    out->pushData((uint8_t *)buff, strlen(buff));
    out->pushData((uint8_t *)"\r\nLS-RGV $ ", sizeof("\r\nLS-RGV $ "));
}

// process main ctl socket cli socket log socket
void socketServerRun()
{
    // for main socket
    if( 1 )
    {
        ...
        static FifoClass in, out;
        BaseType_t lastUseTime = 0;
        int len = 0;
        const int i = 0;
        ...
        case SOCK_ESTABLISHED:
            ...
            if( lastUseTime <= osKernelSysTick() )
            {
                if( lastUseTime == 0 )
                    lastUseTime = osKernelSysTick();

                if( osKernelSysTick() - lastUseTime > timeOut )
                {
                    in.clean();
                    out.clean();
                    close(i);
    ...
    if(1)
    {
        ...
        case SOCK_ESTABLISHED:
            ...
            taskENTER_CRITICAL();
            {
                len = debugFifoBuff.available();
                if (len)
                {
                    debugFifoBuff.popData(buffer, len);
                }
            }
            taskEXIT_CRITICAL();
            if( len )
    ...
    {
        static uint8_t buffer[2048];
        static FifoClass stdinBuff, stdoutBuff;
        static BaseType_t lastUseTime = 0;
        int len = 0;
        const int i = 2;
        ...
        static int CmdStringIndex = 0;
        static BaseType_t xMoreDataToFollow;
        ...
        case SOCK_ESTABLISHED:
            ...
            len = stdoutBuff.available();
            if (len)
            {
                stdoutBuff.popData(buffer, len);
                send(i, buffer, len);
            }
            if( lastUseTime <= osKernelSysTick() )
            {
